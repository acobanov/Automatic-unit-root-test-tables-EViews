subroutine kpss(string %final_name,string %output,string %var_names)
'************************************************************************************************************************************
' Augmented Dickey - Fuller (ADF) unit root tests TABLE
'*************************************************************************************************************************************
 			...
 			for %test {%tests}
				for %model {%models}
					for %dif {%difs}
						...
						!critical_1=@val({%name1}(8,5))
						!critical_5=@val({%name1}(9,5))
						!critical_10=@val({%name1}(10,5))
						!test_statistic=@val({%name1}(7,5))
